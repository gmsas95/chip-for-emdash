...
describe("Commerce public order access", () => {
  it("returns a hosted checkout token and requires it for order lookup", async () => {
    ...
    let providerOrder: Record<string, unknown> | undefined;
    const plugin = createPlugin({ paymentProviders: { chip: { createPayment: async ({ order }) => { providerOrder = order as unknown as Record<string, unknown>; return { checkoutUrl: "https://chip.test/checkout" }; } } } });
    ...
    expect(providerOrder).not.toHaveProperty("orderAccessToken");
